  ...
  const DetailModal = ({ type, onClose }) => {
    const contentData = {
      ...
      notices: {
        ...
        note: "Apply for a Bonafide Certificate via the AI Assistant."
      }
    };
    ...
    return (
      <div className="min-h-screen bg-[#0f172a] text-white p-6 md:p-12 font-sans">
        <header className="max-w-6xl mx-auto flex justify-between items-center mb-12">
          {/* UPDATED HEADING */}
          <h1 className="text-3xl font-black text-blue-500 tracking-tighter uppercase">campus-sync ai</h1>
        ...
        {/* TOP: AI ASSISTANT BANNER */}
        <div className="max-w-6xl mx-auto bg-gradient-to-r from-blue-600 to-indigo-800 p-12 rounded-[3rem] shadow-2xl mb-12">
        ...
        <div className="flex-1 bg-slate-900/90 rounded-[2.5rem] border border-slate-800 flex flex-col overflow-hidden shadow-2xl">
          ...
          <div className="p-4 bg-slate-900 border-t border-slate-800">
            <div className="relative flex items-center max-w-2xl mx-auto w-full">
              <input className="w-full bg-slate-950 border border-slate-800 p-4 pr-16 rounded-2xl outline-none focus:border-blue-500 transition-all" value={input} disabled={isTyping} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend()} placeholder={isTyping ? "AI is typing..." : "Type anything..."} />
              <button onClick={() => handleSend()} disabled={isTyping} className="absolute right-2 bg-blue-600 p-3 rounded-xl shadow-lg active:scale-95"><Send size={20} /></button>
            </div>
          </div>
        </div>
      </div>
